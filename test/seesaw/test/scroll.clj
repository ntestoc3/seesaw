...
(deftest scroll!*-test
  (testing "given an arbitrary component"
    (testing "can scroll to :top"
      (let [p (canvas)
            s (scrollable p)]
        (scroll!* p :to :top)))
    (testing "can scroll to :bottom"
      (let [p (canvas)
            s (scrollable p)]
        (scroll!* p :to :bottom)))
    (testing "can scroll to a java.awt.Point"
      (let [p (canvas)
            s (scrollable p)]
        (scroll!* p :to (java.awt.Point. 20 20))))
    (testing "can scroll to a java.awt.Rectangle"
      (let [p (canvas)
            s (scrollable p)]
        (scroll!* p :to (java.awt.Rectangle. 20 20 10 10))))
    (testing "can scroll to [:point x y]"
      (let [p (canvas)
            s (scrollable p)]
        (scroll!* p :to [:point 10 10])))
    (testing "can scroll to [:rect x y w h]"
      (let [p (canvas)
            s (scrollable p)]
        (scroll!* p :to [:rect 10 10 20 20]))))

  (testing "given a listbox (JList)"
    (testing "can scroll to [:row n]"
  ...
  (testing "given a table (JTable)"
    (testing "can scroll to [:row n]"
      (let [t (make-test-table)
            s (scrollable t)]
        (scroll!* t :to [:row 2])))
    (testing "can scroll to [:column n]"
      (let [t (make-test-table)
            s (scrollable t)]
        (scroll!* t :to [:column 1])))
    (testing "can scroll to [:cell row col]"
      (let [t (make-test-table)
            s (scrollable t)]
        (scroll!* t :to [:cell 3 1]))))

  (testing "given a test component"
    (testing "can scroll to [:line n]"
      (let [t (text :multi-line? true :text "\n\n\n\n")
            s (scrollable t)]
        (scroll!* t :to [:line 2])
        (is (= 2 (.getCaretPosition t)))))
    (testing "can scroll to [:position n]"
      (let [t (text :multi-line? true :text "\n\n\n\n")
            s (scrollable t)]
        (scroll!* t :to [:position 4])
        (is (= 4 (.getCaretPosition t)))))))
